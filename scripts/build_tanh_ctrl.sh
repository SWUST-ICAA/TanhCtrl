...
PYTHON_BUILD_STUB="${WORKSPACE_ROOT}/build/tanh_ctrl/ament_cmake_python/tanh_ctrl/tanh_ctrl"
...
if [[ -d "${PYTHON_BUILD_STUB}" && ! -L "${PYTHON_BUILD_STUB}" ]]; then
  echo "Removing stale Python build directory: ${PYTHON_BUILD_STUB}"
  rm -rf "${PYTHON_BUILD_STUB}"
fi
...
exec colcon build --packages-select tanh_ctrl --symlink-install "$@"
