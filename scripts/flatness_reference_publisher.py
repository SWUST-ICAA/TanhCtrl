...
from tanh_ctrl.msg import FlatTrajectoryReference
...
def omega_from_speed(radius: float, speed: float) -> float:
    return float(speed) / max(float(radius), 1e-6)


def omega_from_period(period_s: float) -> float:
    return 2.0 * math.pi / max(float(period_s), 1e-6)
...
class FlatnessReferencePublisher(Node):
    def __init__(self) -> None:
        super().__init__("flatness_reference_publisher")

        self.declare_parameter("topics.reference", "/tanh_ctrl/reference")
        self.declare_parameter("topics.start_tracking", "/mission/start_tracking")
        self.declare_parameter("planner.publish_rate_hz", 100.0)
        self.declare_parameter("planner.trajectory_type", "circle")
        ...
        self.declare_parameter("planner.height_offset_ned", 0.0)
