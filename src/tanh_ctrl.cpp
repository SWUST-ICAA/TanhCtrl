...
AttitudeReference computeAttitudeReference(const Eigen::Vector3d &desired_thrust_vector_ned,
                                           const TrajectoryRef &ref)
{
  ...
  attitude_reference.torque_body = ref.has_torque_feedforward && ref.torque_body.allFinite()
                                     ? ref.torque_body
                                     : Eigen::Vector3d::Zero();
...
Eigen::Vector4d allocateMotorForces(const AllocationParams &params, double thrust_total,
...
Eigen::Vector4d forcesToMotorControls(const Eigen::Vector4d &forces, double motor_force_max,
                                      double thrust_model_factor)
{
...
{
  return Eigen::Vector3d(sanitizeCutoff(cutoff_hz.x()), sanitizeCutoff(cutoff_hz.y()),
...
double tanh_ctrl::lowPassScalar(double x, double cutoff_hz, double dt, double *state,
                                bool *initialized)
{
...
Eigen::Vector3d tanh_ctrl::lowPassVec3(const Eigen::Vector3d &x, double cutoff_hz, double dt,
...
Eigen::Vector3d tanh_ctrl::lowPassVec3(const Eigen::Vector3d &x, const Eigen::Vector3d &cutoff_hz,
...
bool tanh_ctrl::compute(const VehicleState &state, const TrajectoryRef &ref, double dt,
                        ControlOutput *out)
{
  if (!out || !ref.valid)
  {
...
void tanh_ctrl::computePosition(const VehicleState &state, const TrajectoryRef &ref, double dt,
...
void tanh_ctrl::computeAttitude(const VehicleState &state,
                                ...
                                Eigen::Vector3d *torque_body)
{
  ...
  const Eigen::Vector3d desired_angular_velocity_body =
    attitude_reference.has_angular_velocity_feedforward ? attitude_reference.angular_velocity_body
                                                        : Eigen::Vector3d::Zero();
